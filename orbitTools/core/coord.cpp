...
#define _snprintf_s(a,b,c,...) snprintf(a,b,c,__VA_ARGS__) // Included to port from Win. to Unix.
...
string cGeo::ToString() const
{
   ...
   _snprintf_s(sz, BUF_SIZE, 
               ...
               AltitudeKm() * 1000.0); // ---------------------------------------------------------- (C. Araguz) : Note that this is not UNIX compatible. See define above.
