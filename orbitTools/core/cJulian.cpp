...
cJulian::cJulian(time_t time)
{
   ...
   //gmtime_s(&stm, &time);
   gmtime_r(&time, &stm); // Changed by C. Araguz (carles.araguz@gmail.com) - Note that this change was necessary to port these libraries from Win. to UNIX. 
...
time_t cJulian::ToTime() const
{
   ...
   if (tEpoch != -1)
   {
      ...
      //_get_timezone(&secDelta); ------------------------------------------------------------------ Old version (Windows compatible)
      /** MODIFICATION BY: C. Araguz ***************************************************************
       *    The previous call to _get_timezone has been replaced by the following section to port 
       *    the libraries from Win to Unix. 
       */
